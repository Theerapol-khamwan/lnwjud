...
describe('Windows desktop packaging', () => {
  it('pins the product release to v4.8.31', async () => {
    ...
    expect(rootPackage.version).toBe('4.8.31');
    expect(desktopPackage.version).toBe('4.8.31');
  });
  ...
  it('keeps every workspace package and runtime version aligned', async () => {
    ...
    for (const packagePath of packagePaths) {
      ...
      expect(packageJson.version, packagePath).toBe('4.8.31');
    }
    ...
    expect(ipcContracts).toContain("APP_VERSION = '4.8.31'");
    expect(shared).toContain("APP_VERSION = '4.8.31'");
  });
