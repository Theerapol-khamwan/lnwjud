...
describe('Windows desktop packaging', () => {
  it('pins the product release to v4.28.0', async () => {
    ...
    expect(rootPackage.version).toBe('4.28.0');
    expect(desktopPackage.version).toBe('4.28.0');
  });
  ...
  it('publishes complete desktop application metadata', async () => {
    ...
    expect(desktopPackage.description).toBe('Windows-first local AI-agent runtime and MCP gateway with 229 total tool definitions.');
