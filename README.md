...
<p align="center">
  ...
  <em>231 total tool definitions for local files, Git, processes, Windows automation, WSL, browser control, durable goal continuation, indexing, observability, and extensibility; 195 are advertised by default and 202 when Codex delegation plus Agent Swarm is enabled.</em>
...
5. Confirm that the default runtime exposes **195 tools** (or **202** when Codex delegation is explicitly enabled) and run a read-only
...
## Complete MCP tool catalog (231 total definitions; 195 advertised by default; 202 with Codex enabled)
...
| 90 | `claim_scheduled_continuation` | WRITE | default | operational | service_dispatch | Scheduled-wake entrypoint. Claim before workspace mutation; a confirmed cloud wake up to 120 seconds early is accepted so native host jitter does not consume the one-time task without handoff. If native task creation was never confirmed, returns receipt_required with handoffReady=false and the wake must reconcile the exact host receipt before mutating or returning. A one-time task that is firing is treated as a consumed wake ticket: on an active-worker collision, claim atomically supersedes that ticket and returns successor_required with a fresh +2-minute cloud scheduleRequest, handoffReady=false, and currentWakeMayReturn=false. Create that fresh native successor and record its created cloud receipt before the current wake returns; never re-arm the firing task and never count prepared as confirmed. If the outcome is terminal_noop, let the already-firing host task return naturally; do not delete, disable, pause, or reschedule it. Do not mutate the workspace or mark the goal terminal on collision. |
...
| 137 | `plugin_install` | WRITE | no | feature_disabled | truthful_unavailable | Register a declared plugin descriptor after validation and permission evaluation. |
| 138 | `plugin_list` | READ | no | feature_disabled | truthful_unavailable | List installed and enabled plugins. |
| 139 | `plugin_enable` | WRITE | no | feature_disabled | truthful_unavailable | Enable an installed plugin. |
| 140 | `plugin_disable` | WRITE | no | feature_disabled | truthful_unavailable | Disable an installed plugin. |
| 141 | `plugin_remove` | DANGEROUS | no | feature_disabled | truthful_unavailable | Remove an installed plugin. |
...
| 166 | `task_create` | EXECUTE | no | feature_disabled | truthful_unavailable | Create a visible managed runtime task. |
| 167 | `task_status` | READ | no | feature_disabled | truthful_unavailable | Read managed task state. |
| 168 | `task_cancel` | EXECUTE | no | feature_disabled | truthful_unavailable | Cancel a managed runtime task. |
| 169 | `task_result` | READ | no | feature_disabled | truthful_unavailable | Read a managed task result. |
| 170 | `task_list` | READ | no | feature_disabled | truthful_unavailable | List managed runtime tasks. |
...
| 178 | `live_logs_query` | READ | no | feature_disabled | truthful_unavailable | Query structured activity/log metadata with correlation IDs. |
| 179 | `live_logs_status` | READ | no | feature_disabled | truthful_unavailable | Return Live Logs pipeline health and source status. |
