...
export function ToolsPage({ locale, snapshot, loading, onRefresh, onRemediation }: ToolsPageProps): ReactElement {
  ...
  return (
    <section className="panel tools-page" aria-labelledby="tools-heading">
      <div className="section-heading tools-heading"><div><h1 id="tools-heading">{locale === 'th' ? 'เครื่องมือ' : 'Tools'}</h1><p className="page-subtitle">{locale === 'th' ? 'ดูสิ่งที่พร้อมใช้ สิ่งที่ต้องดำเนินการ และเหตุผลจาก runtime จริง' : 'See what is ready, what needs action, and why from live runtime evidence.'}</p></div><button type="button" disabled={loading} onClick={() => { void onRefresh(); }}>{loading ? (locale === 'th' ? 'กำลังตรวจ…' : 'Checking…') : (locale === 'th' ? 'ตรวจใหม่ทั้งหมด' : 'Recheck all')}</button></div>
      ...
      <div className="tool-status-strip" aria-label={locale === 'th' ? 'จำนวนตามสถานะ' : 'Status counts'}>{statuses.map((status) => <button type="button" key={status} className={readiness === status ? 'active' : undefined} onClick={() => setReadiness(readiness === status ? 'all' : status)}><strong>{counts[status]}</strong><span>{coarseReadinessLabel(locale, status)}</span></button>)}</div>
