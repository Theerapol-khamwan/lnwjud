...
interface LiveLogsPageProps {
  ...
  readonly onExport: (source: LogSource, scope: LogScopeSelection, query: string, lineIds: readonly number[]) => Promise<void>;
      ...
      {sources.map((source) => (
        tab === source ? (
          <LogStreamPanel
            ...
            onExport={(scope, query, lineIds) => props.onExport(source, scope, query, lineIds)}
          />
        ) : null
      ))}
    </div>
