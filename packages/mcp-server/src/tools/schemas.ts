...
export const workspaceTreeSchema = z.object({ workspaceId: workspaceIdSchema, path: pathSchema.optional(), maxDepth: z.number().int().min(1).max(8).optional(), maxEntries: z.number().int().min(1).max(5000).optional() }).strict();
...
export const searchFilesSchema = z.object({ workspaceId: workspaceIdSchema, glob: z.string().max(1024).optional(), maxResults: z.number().int().min(1).max(500).optional() }).strict();
...
export const writeFileSchema = z.object({ workspaceId: workspaceIdSchema, path: pathSchema, content: z.string().refine((value) => Buffer.byteLength(value, 'utf8') <= 4 * 1024 * 1024, 'File is too large') }).strict();
export const applyPatchSchema = z.object({ workspaceId: workspaceIdSchema, files: z.array(z.object({ path: pathSchema, content: z.string().refine((value) => Buffer.byteLength(value, 'utf8') <= 4 * 1024 * 1024, 'File is too large') }).strict()).min(1).max(20) }).strict();
