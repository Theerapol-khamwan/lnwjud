...
| 85 | `cancel_goal` | WRITE | no | yes |
| 86 | `list_goals` | READ | yes | no |
| 87 | `prepare_scheduled_continuation` | WRITE | no | no |
| 88 | `record_scheduled_continuation_receipt` | WRITE | no | no |
| 89 | `claim_scheduled_continuation` | WRITE | no | no |
| 90 | `get_scheduled_continuation` | READ | yes | no |
| 91 | `expedite_scheduled_continuation` | WRITE | no | no |
| 92 | `cancel_scheduled_continuation` | WRITE | no | yes |
| 93 | `symbol_search` | READ | yes | no |
| 94 | `find_definition` | READ | yes | no |
| 95 | `find_references` | READ | yes | no |
| 96 | `find_implementations` | READ | yes | no |
| 97 | `call_hierarchy` | READ | yes | no |
| 98 | `import_graph` | READ | yes | no |
| 99 | `dependency_graph` | READ | yes | no |
| 100 | `module_graph` | READ | yes | no |
| 101 | `type_search` | READ | yes | no |
| 102 | `trace_symbol` | READ | yes | no |
| 103 | `context_ranking` | READ | yes | no |
| 104 | `debug_context` | READ | yes | no |
| 105 | `review_context` | READ | yes | no |
| 106 | `change_context` | READ | yes | no |
| 107 | `symbol_context` | READ | yes | no |
| 108 | `test_context` | READ | yes | no |
| 109 | `dependency_context` | READ | yes | no |
| 110 | `git_context` | READ | yes | no |
| 111 | `frontend_context` | READ | yes | no |
| 112 | `backend_context` | READ | yes | no |
| 113 | `route_intent` | READ | yes | no |
| 114 | `recipe_list` | READ | yes | no |
| 115 | `recipe_describe` | READ | yes | no |
| 116 | `recipe_run` | EXECUTE | no | no |
| 117 | `dry_run` | READ | yes | no |
| 118 | `review_changes` | READ | yes | no |
| 119 | `changed_symbols` | READ | yes | no |
| 120 | `affected_modules` | READ | yes | no |
| 121 | `git_history_context` | READ | yes | no |
| 122 | `git_blame_context` | READ | yes | no |
| 123 | `discover_tests` | READ | yes | no |
| 124 | `run_affected_tests` | EXECUTE | no | no |
| 125 | `test_failures` | READ | yes | no |
| 126 | `coverage_context` | READ | yes | no |
| 127 | `test_history` | READ | yes | no |
| 128 | `cache_stats` | READ | yes | no |
| 129 | `cache_clear` | WRITE | no | no |
| 130 | `cache_invalidate` | WRITE | no | no |
| 131 | `hook_list` | READ | yes | no |
| 132 | `hook_register` | WRITE | no | no |
| 133 | `hook_remove` | WRITE | no | no |
| 134 | `skill_match` | READ | yes | no |
| 135 | `skill_load` | READ | yes | no |
| 136 | `plugin_install` | WRITE | no | no |
| 137 | `plugin_list` | READ | yes | no |
| 138 | `plugin_enable` | WRITE | no | no |
| 139 | `plugin_disable` | WRITE | no | no |
| 140 | `plugin_remove` | DANGEROUS | no | yes |
| 141 | `session_context` | READ | yes | no |
| 142 | `session_checkpoint` | WRITE | no | no |
| 143 | `session_resume` | READ | yes | no |
| 144 | `session_history` | READ | yes | no |
| 145 | `response_mode` | READ | yes | no |
| 146 | `inspect_web_app` | READ | yes | no |
| 147 | `debug_ui` | READ | yes | no |
| 148 | `capture_ui_state` | READ | yes | no |
| 149 | `form_context` | READ | yes | no |
| 150 | `network_context` | READ | yes | no |
| 151 | `console_context` | READ | yes | no |
| 152 | `browser_debug_context` | READ | yes | no |
| 153 | `windows_environment` | READ | yes | no |
| 154 | `service_context` | READ | yes | no |
| 155 | `process_context` | READ | yes | no |
| 156 | `port_context` | READ | yes | no |
| 157 | `registry_context` | READ | yes | no |
| 158 | `event_log_context` | READ | yes | no |
| 159 | `installed_runtime_context` | READ | yes | no |
| 160 | `path_context` | READ | yes | no |
| 161 | `startup_context` | READ | yes | no |
| 162 | `mcp_discover` | READ | yes | no |
| 163 | `mcp_health` | READ | yes | no |
| 164 | `mcp_resources` | READ | yes | no |
| 165 | `task_create` | EXECUTE | no | no |
| 166 | `task_status` | READ | yes | no |
| 167 | `task_cancel` | EXECUTE | no | no |
| 168 | `task_result` | READ | yes | no |
| 169 | `task_list` | READ | yes | no |
| 170 | `delegate` | EXECUTE | no | no |
| 171 | `delegate_status` | READ | yes | no |
| 172 | `delegate_cancel` | EXECUTE | no | no |
| 173 | `delegate_result` | READ | yes | no |
| 174 | `parallel_delegate` | EXECUTE | no | no |
| 175 | `permission_check` | READ | yes | no |
| 176 | `permission_profile` | READ | yes | no |
| 177 | `live_logs_query` | READ | yes | no |
| 178 | `live_logs_status` | READ | yes | no |
| 179 | `telemetry_dashboard` | READ | yes | no |
| 180 | `context_economy_stats` | READ | yes | no |
| 181 | `execution_plan` | READ | yes | no |
| 182 | `repo_map` | READ | yes | no |
| 183 | `context_expand` | READ | yes | no |
| 184 | `recovery_status` | READ | yes | no |
| 185 | `tool_schema_list` | READ | yes | no |
| 186 | `tool_schema_register` | WRITE | no | no |
| 187 | `capabilities` | READ | yes | no |
| 188 | `tool_search` | READ | yes | no |
| 189 | `tool_dynamic_filter` | READ | yes | no |
| 190 | `tool_describe` | READ | yes | no |
| 191 | `tool_categories` | READ | yes | no |
| 192 | `tool_function_find` | READ | yes | no |
| 193 | `tool_aliases` | READ | yes | no |
| 194 | `mcp_hub` | READ | yes | no |
| 195 | `dev_context` | READ | yes | no |
| 196 | `recipe_catalog` | READ | yes | no |
| 197 | `capture_screenshot` | READ | yes | no |
| 198 | `compare_screenshot` | READ | yes | no |
| 199 | `dom_snapshot` | READ | yes | no |
| 200 | `layout_metadata` | READ | yes | no |
| 201 | `visual_context` | READ | yes | no |
| 202 | `inspect_workbook` | READ | yes | no |
| 203 | `compare_workbook_layout` | READ | yes | no |
| 204 | `render_excel_preview` | READ | yes | no |
| 205 | `inspect_pdf` | READ | yes | no |
| 206 | `compare_pdf_pages` | READ | yes | no |
| 207 | `project_profile_get` | READ | yes | no |
| 208 | `project_profile_set` | WRITE | no | no |
| 209 | `handoff_context` | READ | yes | no |
| 210 | `benchmark_run` | EXECUTE | no | no |
| 211 | `regression_report` | READ | yes | no |
| 212 | `sandbox_exec` | EXECUTE | no | no |
| 213 | `event_watch` | EXECUTE | no | no |
| 214 | `crash_trace` | READ | yes | no |
| 215 | `lsp_diagnostics` | READ | yes | no |
| 216 | `lsp_rename` | WRITE | no | no |
| 217 | `debug_attach` | EXECUTE | no | no |
| 218 | `debug_step` | EXECUTE | no | no |
| 219 | `git_worktree_spawn` | WRITE | no | no |
| 220 | `git_worktree_remove` | DANGEROUS | no | yes |
| 221 | `db_inspect` | READ | yes | no |
| 222 | `db_query` | READ | yes | no |
| 223 | `office_ppt` | WRITE | no | no |
| 224 | `office_outlook` | READ | yes | no |
| 225 | `pdf_extract_tables` | READ | yes | no |
| 226 | `docx_merge` | WRITE | no | no |
| 227 | `self_heal_plan` | READ | yes | no |
| 228 | `self_heal_apply` | DANGEROUS | no | yes |
| 229 | `skills_import` | WRITE | no | no |
| 230 | `agent_swarm_run` | EXECUTE | no | no |
| 231 | `tool_batch` | EXECUTE | no | yes |
