...
เมื่อ successor ตื่นต้องเรียก `claim_scheduled_continuation` ก่อน mutation. Native wake ที่มาก่อน due ไม่เกิน 60 วินาที claim ได้เพื่อทนต่อ scheduling jitter ของ one-time host. ถ้าชน worker ที่ยังทำงาน หรือหลักฐาน worker ไม่แน่นอน ให้คืน `reschedule_required` แล้ว update **native task ID เดิม** เป็น `now+2` และ record `rescheduled`; ห้ามสร้าง task/continuation ใหม่และไม่มี retry cap. ถ้าพบ lease ค้างแต่ไม่มี worker จริง ต้องมี trustworthy no-worker probe สองรอบห่างอย่างน้อย 120 วินาที โดย goal revision, lease generation และ activity sequence ไม่เปลี่ยน, ไม่มี live fenced call และ active task IDs terminal/absent จึง CAS takeover เป็น `orphan_recovered`. หลักฐาน unknown ห้าม force-unlock.
