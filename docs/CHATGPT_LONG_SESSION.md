...
เมื่อ run ปัจจุบันยังไม่เสร็จและผู้ใช้เปิดใช้ rolling continuation ให้ `prepare_scheduled_continuation` checkpoint + จอง successor แบบ **one-time/cloud** ด้วย delay แบบ adaptive 2–25 นาที และสร้าง native ChatGPT Scheduled Task เดียวในแชทเดิม จากนั้นบันทึก `created` receipt พร้อม native task ID และ `runsOn: cloud` ก่อนทำงานต่อ. 25 นาทีเป็นค่าเริ่มต้น/เพดาน watchdog สำหรับงานยาว ไม่ใช่ fixed cadence; ใช้ประมาณ 10 นาทีสำหรับ bounded phase และ 5 นาทีสำหรับ final verification/packaging ที่ใกล้เสร็จ. Run เดิมต้องทำงานต่อ ไม่คืน control เพียงเพราะสร้าง successor แล้ว. ถ้า host turn จำเป็นต้องจบขณะ goal ยัง active ให้เลื่อน task เดิมเป็น `now+2` ก่อนจบ turn.
...
เมื่อ successor ตื่นต้องเรียก `claim_scheduled_continuation` ก่อน mutation. Native wake ที่มาก่อน due ไม่เกิน 120 วินาที claim ได้เพื่อทนต่อ scheduling jitter ของ one-time host. ถ้า host metadata ยืนยันภายหลังว่า exact native one-time task รัน/ถูก consume ไปแล้ว แต่ durable continuation ยัง pending/live เพราะ claim ไม่จบ ให้ record `consumed` พร้อม exact native host run receipt; นี่เป็นการยืนยันว่า task ไม่ได้ pending แล้วเท่านั้น ไม่ใช่ goal completion และ goal ที่ยัง active ต้อง reserve/create successor ใหม่หลัง reconciliation. ถ้าชน worker ที่ยังทำงาน หรือหลักฐาน worker ไม่แน่นอน ให้คืน `reschedule_required` แล้ว update **native task ID เดิม** เป็น `now+2` และ record `rescheduled`; ห้ามสร้าง task/continuation ใหม่และไม่มี retry cap. ถ้าพบ lease ค้างแต่ไม่มี worker จริง ต้องมี trustworthy no-worker probe สองรอบห่างอย่างน้อย 120 วินาที โดย goal revision, lease generation และ activity sequence ไม่เปลี่ยน, ไม่มี live fenced call และ active task IDs terminal/absent จึง CAS takeover เป็น `orphan_recovered`. หลักฐาน unknown ห้าม force-unlock.

ถ้าพบ enumerated handoff risk ระหว่าง worker ปัจจุบันยังถือ lease หรือ host turn ต้องคืน control ทั้งที่ goal ยัง active ให้ `expedite_scheduled_continuation` ดึง **task เดิม** มา `now+2`. ระหว่างมี durable background task ให้เก็บ task ID ใน checkpoint และใช้ bounded wait จนอ่าน terminal result; ห้ามส่ง final completion จาก progress log. เมื่อ acceptance ครบ ให้ `finish_goal` แล้วอ่าน `get_goal` ยืนยัน terminal ก่อนตอบผู้ใช้เสมอ แม้ผู้ใช้ปิด scheduled continuation ระหว่างทาง เพราะ opt-out ยกเลิกเฉพาะ successor ไม่ได้ปิด goal. จากนั้นลบ exact native task ตาม cancellation instruction, record `cancelled` พร้อม native host deletion receipt จาก task ID เดิม และอ่านสถานะกลับให้ได้ `cancelled`. ถ้าลบไม่ได้/ไม่แน่ใจให้คง `cancel_required`, `cancel_failed` หรือ `cancel_uncertain` ตามจริง. ห้าม fallback ไป Windows Task Scheduler, OS cron, shell timer, recurrence, browser automation หรือ undocumented OpenAI API.

Timeline อ้างอิง: `claim T+00 -> successor T+25` สำหรับงานเปิดกว้าง; ถ้า phase ท้ายคาดว่าเสร็จเร็วให้สร้าง `T+05`; `terminal T+03 -> finish_goal -> cancel exact successor`; ถ้า turn ต้องจบที่ T+03 ทั้งที่ยัง active ให้ **task เดิม** เป็น `T+05`; collision ที่ due ให้ task เดิม `now+2`; second unchanged trustworthy no-worker probe -> `orphan_recovered` + higher lease generation -> successor ใหม่ตาม delay ที่เหมาะกับงานคงเหลือ.
