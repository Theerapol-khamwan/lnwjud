...
เมื่อ run ปัจจุบันยังไม่เสร็จและผู้ใช้เปิดใช้ rolling continuation ให้ `prepare_scheduled_continuation` checkpoint + จอง successor แบบ **one-time/cloud** ด้วย delay แบบ adaptive 2–25 นาที และสร้าง native ChatGPT Scheduled Task เดียวในแชทเดิม จากนั้นบันทึก `created` receipt พร้อม native task ID และ `runsOn: cloud` ทันที. `dueAt` เป็น canonical absolute instant แต่ VEVENT ที่ส่งให้ native host ต้องมี explicit IANA `TZID` เพื่อป้องกัน host ตีความ UTC `Z` เป็น local wall-clock ผิดเวลา. ถ้า native create ล้มเหลวหรือไม่แน่นอน ให้ record ตามจริงและ worker ที่ยังมี lease ถูกต้องทำ fenced work ต่อได้จนถึง handoff deadline; failure ของ scheduler ไม่ใช่เหตุให้หยุด goal. ถ้าไม่ระบุ delay ระบบจะ fail-safe ที่ **+2 นาที**; 5/10/25 นาทีเป็น watchdog ที่ต้องเลือก explicitly เฉพาะเมื่อ run เดิมจะทำงานต่อจริง โดย 25 นาทีเป็นเพียงค่าสูงสุด. ถ้า host turn กำลังจะจบหรือหลัง response จะไม่มี worker เหลือ ให้สร้าง successor ที่ +2 โดยตรง หรือเลื่อน task เดิมที่ยืนยันแล้วเป็น `now+2` และ record reschedule ก่อนจบ turn. ห้ามคืน control ขณะ goal ยัง active และยังไม่มี confirmed cloud successor เว้นแต่ผู้ใช้ปิด scheduling โดยชัดเจน.
...
เมื่อ successor ตื่นต้องเรียก `claim_scheduled_continuation` ก่อน mutation. Native wake ที่มาก่อน due ไม่เกิน 120 วินาที claim ได้เพื่อทนต่อ scheduling jitter ของ one-time host. ถ้าได้ `acquired` ธุรกรรม claim จะ reserve successor รุ่นถัดไปเป็น `prepared` ที่ fail-safe +2 นาทีพร้อมกันและคืน `scheduleRequest`; worker ต้องสร้าง/record native task จาก request นี้และห้ามเรียก `prepare_scheduled_continuation` ซ้ำ. ถ้า response ขาดหลัง claim การเรียกซ้ำจะได้ `successor_required` พร้อม reservation เดิม. ถ้า host metadata ยืนยันภายหลังว่า exact native one-time task รัน/ถูก consume ไปแล้ว แต่ durable continuation ยัง pending/live เพราะ claim ไม่จบ ให้ record `consumed` พร้อม exact native host run receipt; นี่เป็นการยืนยันว่า task ไม่ได้ pending แล้วเท่านั้น ไม่ใช่ goal completion และ goal ที่ยัง active ต้อง reserve/create successor ใหม่หลัง reconciliation. ถ้าชน worker ที่ยังทำงาน, lease หมดอายุแต่มี `blocking_job` running หรือหลักฐาน worker ไม่แน่นอน ให้ใช้ `reschedule_required` อัปเดต **native task เดิมที่ยืนยันแล้ว** ไป `now+2`, คง task enabled และ record rescheduled receipt; ห้ามสร้าง replacement task. ถ้าพบ lease ค้างแต่ไม่มี worker จริง ต้องมี trustworthy no-worker probe สองรอบห่างอย่างน้อย 120 วินาที โดย goal revision, lease generation และ activity sequence ไม่เปลี่ยน, ไม่มี live fenced call และ blocking task ทุกตัว terminal/absent จึง CAS takeover เป็น `orphan_recovered`. หลักฐาน unknown ห้าม force-unlock.
