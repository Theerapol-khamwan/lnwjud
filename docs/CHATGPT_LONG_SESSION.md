...
เมื่อ run ปัจจุบันยังไม่เสร็จและผู้ใช้เปิดใช้ rolling continuation ให้ `prepare_scheduled_continuation` checkpoint + จอง successor แบบ **one-time/cloud** ที่ T+25 นาที และสร้าง native ChatGPT Scheduled Task เดียวในแชทเดิม จากนั้นบันทึก `created` receipt พร้อม native task ID และ `runsOn: cloud` ก่อนทำงานยาว. ที่ T+15 ถ้า worker ยังปกติให้คง T+25 เดิม; elapsed time หรือแค่งานยังไม่เสร็จไม่ใช่เหตุให้เลื่อน task.
...
เมื่อ successor ตื่นต้องเรียก `claim_scheduled_continuation` ก่อน mutation. ถ้าชน worker ที่ยังทำงาน หรือหลักฐาน worker ไม่แน่นอน ให้คืน `reschedule_required` แล้ว update **native task ID เดิม** เป็น `now+2` และ record `rescheduled`; ห้ามสร้าง task/continuation ใหม่และไม่มี retry cap. ถ้าพบ lease ค้างแต่ไม่มี worker จริง ต้องมี trustworthy no-worker probe สองรอบห่างอย่างน้อย 120 วินาที โดย goal revision, lease generation และ activity sequence ไม่เปลี่ยน, ไม่มี live fenced call และ active task IDs terminal/absent จึง CAS takeover เป็น `orphan_recovered`. หลักฐาน unknown ห้าม force-unlock.

ถ้าพบ enumerated handoff risk ระหว่าง worker ปัจจุบันยังถือ lease ให้ `expedite_scheduled_continuation` ดึง **task เดิม** มา `now+2`; T+15 อย่างเดียวไม่ใช่ handoff risk. เมื่อ goal เสร็จก่อน successor ให้ `finish_goal` ก่อน ลบ exact native task ตาม cancellation instruction แล้ว record `cancelled` พร้อม native host deletion receipt จาก task ID เดิม จากนั้นอ่านสถานะกลับและต้องได้ `cancelled` ก่อนรายงานว่ายกเลิกสำเร็จ ถ้าลบไม่ได้/ไม่แน่ใจให้คง `cancel_required`, `cancel_failed` หรือ `cancel_uncertain` ตามจริง. ห้าม fallback ไป Windows Task Scheduler, OS cron, shell timer, recurrence, browser automation หรือ undocumented OpenAI API.

Timeline อ้างอิง: `claim T+00 -> successor T+25`; `terminal T+12 -> cancel exact successor`; `healthy T+15 -> keep T+25`; `handoff risk T+15 -> same task T+17`; `collision T+25 -> same task T+27`; second unchanged trustworthy no-worker probe -> `orphan_recovered` + higher lease generation -> new successor T+25.
